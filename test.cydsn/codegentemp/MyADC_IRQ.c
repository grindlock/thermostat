...
{
    CyRamVectors[CYINT_IRQ_BASE + MyADC_IRQ__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + MyADC_IRQ__INTC_NUMBER];
}
...
void MyADC_IRQ_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((MyADC_IRQ__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *MyADC_IRQ_INTC_PRIOR = (*MyADC_IRQ_INTC_PRIOR & (uint32)(~MyADC_IRQ__INTC_PRIOR_MASK)) |
...
uint8 MyADC_IRQ_GetPriority(void)
{
    ...
    priority = (*MyADC_IRQ_INTC_PRIOR & MyADC_IRQ__INTC_PRIOR_MASK) >> priorityOffset;
