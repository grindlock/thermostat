...
uint16 I2C_1_offsetBuf1; /* Current offset within buffer 1 */
uint16 I2C_1_indexBuf1;             /* Current index within buffer 1  */
    ...
    uint16 I2C_1_offsetBuf2; /* Current offset within buffer 2 */
...
***************************************/
    ...
    ****************************************************************************//**
    *
    *  Configures the I2C_1 for EZI2C operation.
    *
    ...
    *  After initializing the I2C_1 in EZI2C mode using this function, 
...
*  - I2C_1_EZI2C_STATUS_READ1 - Read transfer complete. The transfer 
...
*  - I2C_1_EZI2C_STATUS_WRITE1 - Write transfer complete. The buffer 
...
*  - I2C_1_EZI2C_STATUS_READ2 - Read transfer complete. The transfer
*    used the secondary slave address. The error condition status bit must be 
*    checked to ensure that read transfer was completed successfully.
*  - I2C_1_EZI2C_STATUS_WRITE2 - Write transfer complete. The buffer
*    content was modified. The transfer used the secondary slave address. 
*    The error condition status bit must be checked to ensure that write 
*    transfer was completed successfully.
*  - I2C_1_EZI2C_STATUS_BUSY - A transfer intended for the primary 
...
*  - I2C_1_EZI2C_STATUS_ERR - An error occurred during a transfer 
...
*    I2C_1_EZI2C_STATUS_ERR and I2C_1_EZI2C_STATUS_WRITE1/2 
*    is set. It is recommended to discard buffer content in this case.
*
* \globalvars
*  I2C_1_curStatus - used to store the current status of the EZI2C 
*  slave.
*
*******************************************************************************/
...
*
* \globalvars
*  I2C_1_addrBuf1 - used to store the primary 7-bit slave address 
*  value.
*
*******************************************************************************/
void I2C_1_EzI2CSetAddress1(uint32 address)
{
    if(I2C_1_SECONDARY_ADDRESS_ENABLE)
    {
        ...
        I2C_1_RX_MATCH_REG = 
                        I2C_1_EzI2CUpdateRxMatchReg(address, (uint32) I2C_1_addrBuf2);
    }
    else
    {
        uint32 matchReg;
        ...
        /* Set address. */
        matchReg &= (uint32) ~I2C_1_RX_MATCH_ADDR_MASK;
...
*
* \globalvars
*  I2C_1_dataBuffer1 – the pointer to the buffer to be exposed to the
*  master on a primary address.
*  I2C_1_bufSizeBuf1 - the size of the buffer to be exposed to the 
*  master on a primary address.
*  I2C_1_protectBuf1 - the start index of the read-only region in the
...
void I2C_1_EzI2CSetReadBoundaryBuffer1(uint32 rwBoundary)
{
    ...
    *
    * \globalvars
    *  I2C_1_addrBuf2 - used to store the secondary 7-bit slave address 
    *  value.
    *
    *******************************************************************************/
    void I2C_1_EzI2CSetAddress2(uint32 address)
    {
        ...
        I2C_1_RX_MATCH_REG = 
                        I2C_1_EzI2CUpdateRxMatchReg((uint32) I2C_1_addrBuf1, address);
    }
    ...
    *
    * \globalvars
    *  I2C_1_dataBuffer2 – the pointer to the buffer to be exposed to the
    *  master on a secondary address.
    *  I2C_1_bufSizeBuf2 - the size of the buffer to be exposed to the 
    *  master on a secondary address.
    *  I2C_1_protectBuf2 - the start index of the read-only region in the
